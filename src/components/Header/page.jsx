...
export default function Header() {
  ...
  // کنترل باز و بسته شدن زیرمنو موبایل
  const toggleDropdown = (index) => {
    setOpenDropdown(openDropdown === index ? null : index);
  };
    ...
    <div dir="rtl" className="w-full relative z-50">
      <header className="w-full p-6 flex justify-between items-center bg-white text-black shadow-md">
        <div className="flex gap-3 items-center">
          ...
          {/* لوگو - فقط در md به بالا نمایش داده شود */}
          <img
            src="https://assets.khanoumi.com/4.91.0.0/_toad/images/brand/logo.svg?v=1.1"
            alt="logo"
        ...
        <div className="hidden md:flex items-center gap-2 text-[18px] rounded-md px-4 w-1/3 py-2 border border-gray-300">
          ...
          <img className="flex md:hidden" src="https://assets.khanoumi.com/4.91.0.0/_toad/images/brand/logo.svg?v=1.1" alt="" />
        ...
        <div className="flex gap-6 items-center">
          ...
          <Link href="/Basket" className="relative cursor-pointer" aria-label="Shopping Basket">
            {/* تعداد کالاهای سبد به صورت badge */}
            {totalCount > 0 && (
      ...
      {/* منوی موبایل - فقط وقتی موبایل منو بازه */}
      {mobileMenu && (
  <>
    {/* بک‌گراند نیمه شفاف پشت منو */}
    <div
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <img src="https://assets.khanoumi.com/4.91.0.0/_toad/images/brand/logo.svg?v=1.1" alt="" />
        <button
          onClick={() => setMobileMenu(false)}
        ...
        <ul className="flex gap-4 p-4 items-center justify-start">
          {categories.map((item, index) => (
            ...
            >
              {item.name}
              {item.sub && <FiChevronDown className="inline ml-2" />}
              {item.sub && hovered === index && index !== categories.length - 1 && (
                <div
                ...
                >
                  {item.sub.map((subItem, subIndex) => (
                    <div
                      key={subIndex}
                      className={`py-2 text-center ${
                        subIndex !== item.sub.length - 1 ? "border-b border-gray-200" : ""
                      } hover:bg-gray-100 cursor-pointer`}
                    >
                      {subItem}
                    </div>
